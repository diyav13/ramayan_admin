...
type ChapterThumbnailFieldProps = {
  ...
  /** Pass when editing an existing chapter so uploads land under that chapter folder. */
  chapterId?: string;
...
/**
 * Chapter thumbnail picker — presigned PUT to S3, then stores the public URL.
...
export function ChapterThumbnailField({
  ...
  async function handleFileChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const publicUrl = await uploadService.chapterThumbnail(file, chapterId);
      revokeLocalPreview();
      onChange(publicUrl);
    } catch (err) {
      revokeLocalPreview();
